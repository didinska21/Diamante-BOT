...
const logBox = blessed.log({
  ...
  height: "100%-23",
  border: { type: "line", fg: "cyan" },
  style: { fg: "white", bg: "default", scrollbar: { bg: "cyan" } },
  scrollable: true,
  alwaysScroll: true,
  scrollbar: true,
  tags: true,
...
const inputBox = blessed.textbox({
  ...
  style: { fg: "white", bg: "default", focus: { border: { fg: "yellow" } } },
  label: " Enter Choice "
});
...
figlet.text("DIAM BOT", { font: "Standard" }, (err, data) => {
  if (!err) {
...
function updateStatus() {
  const status = activityRunning
    ...
    : isCycleRunning
      ? `${loadingSpinner[spinnerIndex]} {yellow-fg}Waiting{/yellow-fg}`
      : "{green-fg}Idle{/green-fg}";
  const statusText = `Status: ${status} | Accounts: ${addresses.length} | Send Config: ${dailyActivityConfig.sendDiamRepetitions}x | Amount: ${dailyActivityConfig.minSendAmount}-${dailyActivityConfig.maxSendAmount} DIAM`;
...
function updateMenu() {
  let menuContent = "\n";
  menuContent += "{bold}{cyan-fg}═══════════════════════════════════════════════════{/cyan-fg}{/bold}\n";
  if (isCycleRunning) {
    menuContent += "  {yellow-fg}1.{/yellow-fg} Stop Daily Activity\n";
  } else {
    menuContent += "  {yellow-fg}1.{/yellow-fg} Start Auto Daily Activity\n";
  }
  menuContent += "  {yellow-fg}2.{/yellow-fg} Create Auto Referral\n";
  menuContent += "  {yellow-fg}3.{/yellow-fg} Set Send DIAM Repetitions\n";
  menuContent += "  {yellow-fg}4.{/yellow-fg} Set Send Amount Config\n";
  menuContent += "  {yellow-fg}5.{/yellow-fg} Refresh Wallet Info\n";
  menuContent += "  {yellow-fg}6.{/yellow-fg} Clear Logs\n";
  menuContent += "  {yellow-fg}0.{/yellow-fg} Exit\n";
  menuContent += "{bold}{cyan-fg}═══════════════════════════════════════════════════{/cyan-fg}{/bold}\n";
  ...
  switch (choice) {
    case "1":
      if (isCycleRunning) {
        ...
        addLog("Stopping daily activity...", "info");
      } else {
        if (addresses.length === 0) {
          addLog("No addresses loaded. Please check user.txt", "error");
        } else {
          runDailyActivity();
        }
      }
    ...
    case "2":
      addLog("Enter Referral Code:", "info");
      inputBox.readInput((err, refCode) => {
        if (!refCode || !refCode.trim()) {
          addLog("Referral code is required.", "error");
          return;
        }
        addLog("Enter number of accounts (1-100):", "info");
        inputBox.readInput((err, countStr) => {
          const count = parseInt(countStr);
          if (isNaN(count) || count < 1 || count > 100) {
            addLog("Invalid count. Please enter 1-100.", "error");
            return;
          }
          addLog("Enter proxies (comma-separated, optional - press enter to skip):", "info");
          inputBox.readInput((err, proxyStr) => {
            const proxiesForReff = proxyStr ? proxyStr.split(',').map(p => p.trim()).filter(p => p) : [];
            runCreateAutoReff(refCode.trim(), count, proxiesForReff);
          });
        });
      });
      break;
    ...
    case "3":
      addLog("Enter Send DIAM Repetitions (1-1000):", "info");
      inputBox.readInput((err, repStr) => {
        const reps = parseInt(repStr);
        if (isNaN(reps) || reps < 1 || reps > 1000) {
          addLog("Invalid input. Please enter 1-1000.", "error");
          return;
        }
        dailyActivityConfig.sendDiamRepetitions = reps;
        saveConfig();
        addLog(`Send DIAM Config set to ${reps}`, "success");
        updateStatus();
      });
      break;
      
    case "4":
      addLog("Enter Min Send Amount:", "info");
      inputBox.readInput((err, minStr) => {
        const minAmount = parseFloat(minStr);
        if (isNaN(minAmount) || minAmount <= 0) {
          addLog("Invalid min amount.", "error");
          return;
        }
        addLog("Enter Max Send Amount:", "info");
        inputBox.readInput((err, maxStr) => {
          const maxAmount = parseFloat(maxStr);
          if (isNaN(maxAmount) || maxAmount <= 0 || maxAmount <= minAmount) {
            addLog("Invalid max amount. Must be > min amount.", "error");
            return;
          }
          dailyActivityConfig.minSendAmount = minAmount;
          dailyActivityConfig.maxSendAmount = maxAmount;
          saveConfig();
          addLog(`Send Amount Config: ${minAmount} - ${maxAmount} DIAM`, "success");
          updateStatus();
        });
      });
      break;
    ...
    case "5":
      ...
      addLog("=== Wallet Information ===", "info");
      walletData.forEach((data, i) => {
        addLog(`${i + 1}. ${data}`, "info");
      });
    ...
    default:
      addLog("Invalid choice. Please enter 0-6.", "error");
  }
...
async function initialize() {
  ...
  const walletData = await updateWalletData();
  addLog("=== Initial Wallet Information ===", "info");
  walletData.forEach((data, i) => {
    addLog(`${i + 1}. ${data}`, "info");
  });
...
function addLog(message, type = "info") {
  ...
  transactionLogs.push(`{cyan-fg}[${timestamp}]{/cyan-fg} ${coloredMessage}`);
  if (transactionLogs.length > 500) transactionLogs.shift();
  updateLogs();
}
